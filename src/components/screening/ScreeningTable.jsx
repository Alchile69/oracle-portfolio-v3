...
const ScreeningTable = ({ 
  ...
  const loadScreeningData = async () => {
    ...
    try {
      console.log(`🔍 Chargement des données de screening (${maxRows} actifs)...`);
      
      // Obtenir les données réelles via le service
      const screeningData = await screeningService.getScreeningData(null, maxRows);
      
      if (screeningData && screeningData.length > 0) {
        setAssets(screeningData);
        setDataSource('real_data');
        console.log(`✅ ${screeningData.length} actifs chargés avec succès`);
        
        // Afficher les stats du cache
        const cacheStats = screeningService.getCacheStats();
        console.log(`📊 Cache: ${cacheStats.active}/${cacheStats.total} actifs (${cacheStats.hitRate}% hit rate)`);
      } else {
        throw new Error('Aucune donnée reçue des APIs');
      }
    } catch (error) {
      ...
      <div className="screening-header">
        ...
        <div className="screening-stats">
          ...
          <div className="data-source-indicator">
            <div className={`source-dot ${dataSource === 'real_data' ? 'real' : dataSource === 'error' ? 'error' : 'loading'}`}></div>
            <span className="source-text">
              {dataSource === 'real_data' ? 'Données réelles' :
               dataSource === 'error' ? 'Données de démo' :
               'Chargement...'}
            </span>
            {error && (
              <span className="error-text" title={error}>⚠️</span>
            )}
          </div>
        </div>
      </div>
